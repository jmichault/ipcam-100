...
int sample_jpeg_init()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			...
			ret = IMP_Encoder_SetMaxStreamCnt(2 + chn[i].index, 1);
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "IMP_Encoder_SetMaxStreamCnt$(%d) error: %d\n",
							chn[i].index, ret);
				return -1;
			}
			/* Create Channel */
...
int sample_encoder_init()
{
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			imp_chn_attr_tmp = &chn[i].fs_chn_attr;
			memset(&channel_attr, 0, sizeof(IMPEncoderCHNAttr));
channel_attr.xW=imp_chn_attr_tmp->picWidth;
channel_attr.xH=imp_chn_attr_tmp->picHeight;
			enc_attr = &channel_attr.encAttr;
...
/*
			...
			*/
			...
			//rc_attr-->attrRcMode.attrH264Vbr.maxGop = 1 * rc_attr->attrRcMode.attrH264Vbr.outFrmRate.frmRateNum / rc_attr->attrRcMode.attrH264Vbr.outFrmRate.frmRateDen;
			rc_attr->attrRcMode.attrH264Vbr.maxQp = 38;
			rc_attr->attrRcMode.attrH264Vbr.minQp = 15;
			rc_attr->attrRcMode.attrH264Vbr.staticTime = 1;
			rc_attr->attrRcMode.attrH264Vbr.maxBitRate = 100 * (imp_chn_attr_tmp->picWidth * imp_chn_attr_tmp->picHeight) / (1920 * 1080);
...
int sample_do_get_h264_stream(int nr_frames)
{
	...
	for (i = 0; i < nr_frames; i++) {
		...
		if (ret < 0) {
			IMP_LOG_ERR(TAG, "Polling stream timeout1\n");
			continue;
		}
...
int sample_do_get_jpeg_snap(void)
{
	...
	if (ret < 0) {
		IMP_LOG_ERR(TAG, "Polling stream timeout2\n");
		return -1;
	}
...
int sample_get_jpeg_snap()
{
	...
	for (i = 0; i < FS_CHN_NUM; i++) {
		if (chn[i].enable) {
			...
			if (ret < 0) {
				IMP_LOG_ERR(TAG, "Polling stream timeout4\n");
				continue;
			}
