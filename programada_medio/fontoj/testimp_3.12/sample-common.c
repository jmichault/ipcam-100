...
static const int S_RC_METHOD = ENC_RC_MODE_VBR;
	...
	for (i = 0; i <  FS_CHN_NUM; i++) {
		if (chn[i].enable) {
            ...
            if (S_RC_METHOD == ENC_RC_MODE_CBR) {
                ...
                rc_attr->attrRcMode.attrH264Vbr.maxQp = 48;
                rc_attr->attrRcMode.attrH264Vbr.minQp = 15;
                rc_attr->attrRcMode.attrH264Vbr.staticTime = 2;
                rc_attr->attrRcMode.attrH264Vbr.maxBitRate = (double)600.0 * (imp_chn_attr_tmp->picWidth * imp_chn_attr_tmp->picHeight) / (1280 * 720);
